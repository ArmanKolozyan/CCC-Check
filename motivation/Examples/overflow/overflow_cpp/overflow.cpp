...
void Withdraw_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Withdraw_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 67;}
...
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[2]); // line circom 13
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// need to run sub component
...
Withdraw_2_create(1,0,ctx,"main",0);
Withdraw_2_run(0,ctx);
}
