...
pub(super) fn int_to_ff<F: PrimeField>(i: Integer) -> F {
    ...
    F::from_repr_vartime(repr).unwrap_or_else(|| {
        panic!(
            "Failed to convert integer to field element\n\
             Integer value: {i}\n\
             Digit count: {} (repr capacity: {})",
            digits.len(),
            repr.as_ref().len()
        )
    })
    ...
    E::Fr: PrimeFieldBits,
{
    ...
    fn setup(p_data: ProverData, v_data: VerifierData) -> (Self::ProvingKey, Self::VerifyingKey) {
        ...
        let mut rng = bellman_rng();
        let params =
            groth16::generate_random_parameters::<E, _, _>(SynthInput(&p_data, None), &mut rng)
                .unwrap();
    ...
    fn prove(pk: &Self::ProvingKey, witness: &FxHashMap<String, Value>) -> Self::Proof {
        ...
        Proof(
            groth16::create_random_proof(SynthInput(&pk.0, Some(witness)), &pk.1, &mut rng)
                .unwrap(),
        )
    }
...
mod test {
    ...
    fn int_to_ff_random(BlsScalar(i): BlsScalar) -> bool {
        let by_fn = int_to_ff::<Fr>(i.clone());
        let by_str = Fr::from_str_vartime(&format!("{i}")).unwrap();
        by_fn == by_str
    }
