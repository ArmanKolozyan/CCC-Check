...
/// Return true if scalarizing inputs/witnesses is necessary.
pub fn needs_scalarize_inputs(cs: &Computation) -> bool {
    for md in cs.metadata.vars_iter() {
        if !md.committed && !md.sort.is_scalar() {
            return true;
        }
    }
    let has_nonscalar_witness = |t: &Term| {
        if let Op::Witness(_) = t.op() {
            return !check(t).is_scalar();
        }
        false
    };
    for t in cs.terms_postorder() {
        if has_nonscalar_witness(&t) {
            return true;
        }
    }
    for t in cs.precomputes.terms_postorder() {
        if has_nonscalar_witness(&t) {
            return true;
        }
    }
