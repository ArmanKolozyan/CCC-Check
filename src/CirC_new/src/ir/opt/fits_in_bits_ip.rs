...
pub fn fits_in_bits_ip(c: &mut Computation) {
    // Non-datan backends (e.g., bellman) don't support multiple rounds.
    // The Haboeck range check protocol introduces challenges that create
    // 2+ rounds. For multi-output computations, merge outputs and skip
    // the interactive protocol; PfFitsInBits will be handled by R1CS
    // lowering via bit decomposition instead.
    if !cfg!(feature = "datan") && c.outputs.len() > 1 {
        c.outputs = vec![term(AND, std::mem::take(&mut c.outputs))];
        return;
    }

    let mut constraints = Vec::new();

    // Collect constraints from all outputs
    for output in &c.outputs {
        collect_constraints(output, &mut constraints);
    }
    ...
    if c.outputs.len() > 1 {
        // datan multi-output: keep separate outputs for multi-round support
        for output in &mut c.outputs {
            *output = extras::substitute_cache(output, &mut substitution_cache);
        }
        c.outputs.extend(new_assertions);
    } else {
        // Single output: combine assertions with the output
