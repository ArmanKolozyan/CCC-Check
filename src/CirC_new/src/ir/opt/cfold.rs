...
pub fn fold_cache(node: &Term, cache: &mut TermCache<TTerm>, ignore: &[Op]) -> Term {
    ...
    while let Some((t, children_pushed)) = stack.pop() {
        ...
        let new_t_opt = if const_args.len() == args.len() && !t.is_var() {
            // CRITICAL FIX: Bounds check for Op::Select before calling eval_op
            // Without this, fully-constant select operations bypass the bounds check
            // in the else branch (around line 415) and cause panics in eval_op
            if matches!(t.op(), Op::Select) && const_args.len() == 2 {
                match (const_args[0], const_args[1]) {
                    (Value::Array(arr), idx) => {
                        if let Some(idx_u) = idx.as_usize() {
                            if idx_u >= arr.size {
                                // Out of bounds - don't fold, leave as-is for proper error handling
                                None
                            } else {
                    ...
                    _ => Some(const_(eval_op(t.op(), &const_args, &Default::default())))
                }
            } else {
                Some(const_(eval_op(t.op(), &const_args, &Default::default())))
            }
            ...
            match &t.op() {
                ...
                Op::Select => match (get(0).as_array_opt(), get(1).as_value_opt()) {
                    (Some(arr), Some(idx)) => {
                        // Bounds check before attempting select
                        if let Some(idx_u) = idx.as_usize() {
                            if idx_u < arr.size {
                                Some(const_(arr.select(idx)))
                            } else {
                                // Out of bounds - don't fold, leave for proper error handling
                                None
                            }
                        } else {
                            // Non-constant index - attempt select
                            Some(const_(arr.select(idx)))
                        }
                    }
                    _ => None,
