...
impl TupleTree {
    ...
    fn map(self, mut f: impl FnMut(Term) -> Term) -> Self {
        fn rec(t: TupleTree, f: &mut impl FnMut(Term) -> Term) -> TupleTree {
            match t {
                TupleTree::NonTuple(term) => TupleTree::NonTuple(f(term)),
                TupleTree::Tuple(children) => {
                    TupleTree::Tuple(children.into_iter().map(|c| rec(c, f)).collect())
                }
            }
        }
        rec(self, &mut f)
    }
    fn bimap(self, other: Self, mut f: impl FnMut(Term, Term) -> Term) -> Self {
        fn rec(
            a: TupleTree,
            b: TupleTree,
            f: &mut impl FnMut(Term, Term) -> Term,
        ) -> TupleTree {
            match (a, b) {
                (TupleTree::NonTuple(ta), TupleTree::NonTuple(tb)) => {
                    TupleTree::NonTuple(f(ta, tb))
                }
                (TupleTree::Tuple(ta), TupleTree::Tuple(tb)) => {
                ...
                _ => panic!("TupleTree structure mismatch"),
            }
        }
        rec(self, other, &mut f)
    }
    fn transpose_map(vs: Vec<Self>, mut f: impl FnMut(Vec<Term>) -> Term) -> Self {
        fn rec(
            vs: Vec<TupleTree>,
            f: &mut impl FnMut(Vec<Term>) -> Term,
        ) -> TupleTree {
            match &vs[0] {
                TupleTree::NonTuple(_) => {
                    let terms: Vec<Term> = vs
                        .into_iter()
                        .map(|v| v.unwrap_non_tuple())
                        .collect();
                    TupleTree::NonTuple(f(terms))
                }
                TupleTree::Tuple(first) => {
                    let arity = first.len();
                    let mut groups: Vec<Vec<TupleTree>> = vec![Vec::with_capacity(vs.len()); arity];
                    for v in vs {
                        match v {
                            TupleTree::Tuple(children) => {
                    ...
                    TupleTree::Tuple(groups.into_iter().map(|g| rec(g, f)).collect())
                }
            }
        }
        assert!(!vs.is_empty());
...
/// Return true if tuple elimination is needed.
pub fn needs_tuple_elim(cs: &Computation) -> bool {
    for o in cs.outputs.iter() {
        if find_tuple_term(o.clone()).is_some() {
            return true;
        }
    }
    for t in cs.precomputes.outputs().values() {
        if find_tuple_term(t.clone()).is_some() {
            return true;
        }
    }
    false
    ...
    for t in terms {
        ...
        let new_t = match t.op() {
            ...
            Op::Ite => {
                ...
                t.bimap(f, |a, b| term![Op::Ite; c.clone(), a, b])
            }
            Op::Eq => {
                let b = cs.pop().unwrap();
                let a = cs.pop().unwrap();
            ...
            Op::CStore => {
                ...
                let a = cs.pop().unwrap();
                debug_assert!(cs.is_empty());
                a.bimap(v, |a, v| term![Op::CStore; a, i.clone(), v, c.clone()])
            }
            Op::Store => {
                let v = cs.pop().unwrap();
                let i = cs.pop().unwrap().unwrap_non_tuple();
                let a = cs.pop().unwrap();
                debug_assert!(cs.is_empty());
                a.bimap(v, |a, v| term![Op::Store; a, i.clone(), v])
            }
