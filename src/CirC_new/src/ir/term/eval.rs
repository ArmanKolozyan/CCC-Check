...
use super::{
    ...
    PfNaryOp, PfUnOp, Sort, Term, TermMap, Value, Var,
};
...
fn strip_ssa_suffix(name: &str) -> Option<&str> {
    let bytes = name.as_bytes();
    let mut idx = bytes.len();
    while idx > 0 && bytes[idx - 1].is_ascii_digit() {
        idx -= 1;
    }
    if idx == bytes.len() || idx < 2 {
        return None;
    }
    if &name[idx - 2..idx] != "_n" {
        return None;
    }
    Some(&name[..idx - 2])
}

fn coerce_value_for_var(var: &Var, value: &Value) -> Value {
    match (&var.sort, value) {
        (Sort::Field(field), Value::Bool(b)) => {
            Value::Field(field.new_v(Integer::from(if *b { 1 } else { 0 })))
        }
        (Sort::Field(field), Value::BitVector(bv)) => {
            Value::Field(field.new_v(bv.uint().clone()))
        }
        (Sort::Field(field), Value::Int(i)) => Value::Field(field.new_v(i.clone())),
        _ => value.clone(),
    }
}
...
pub fn eval_op(op: &Op, args: &[&Value], var_vals: &FxHashMap<String, Value>) -> Value {
    match op {
        Op::Var(var) => {
            if let Some(v) = var_vals.get(&*var.name) {
                coerce_value_for_var(var, v)
            } else if let Some(base) = strip_ssa_suffix(&var.name) {
                let v = var_vals
                    .get(base)
                    .unwrap_or_else(|| panic!("Missing var: {} in {:?}", var.name, var_vals));
                coerce_value_for_var(var, v)
            } else {
                panic!("Missing var: {} in {:?}", var.name, var_vals)
            }
        }
        ...
        Op::PfFitsInBits(n_bits) => {
            // Field elements are unsigned: check if value fits in n_bits
            // unsigned bits (i.e., value is in [0, 2^n_bits - 1]).
            // significant_bits() returns 0 for 0, which fits in any bit width.
            Value::Bool(args[0].as_pf().i().significant_bits() <= *n_bits as u32)
        }
        // tuple
