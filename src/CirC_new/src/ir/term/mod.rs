...
impl Array {
    ...
    pub fn new(
        ...
        // Validation for array construction
        // Ensure all indices in the map are within bounds [0, size)
        if !map.is_empty() {
            if let Some(max_idx) = map.keys().filter_map(|k| k.as_usize()).max() {
                if max_idx >= size {
                    panic!(
                        "Array construction error: max index {} >= size {}. \
                         Valid indices for size {} are 0..{}. \
                         This indicates a bug in array construction where indices weren't properly remapped.",
                        max_idx, size, size, size - 1
                    );
                }
            }
        }
        ...
        match idx.as_usize() {
            ...
            Some(idx_u) => panic!(
                "IR Array out of range: accessed {}, size is {}\n\
                 ...
                 Array map size: {}\n\
                 Index: {}",
                idx_u, self.size, self.key_sort, self.default, self.map.len(), idx
            ),
            _ => {
                // Provide more helpful error message for likely negative indices
                if let Value::Field(f) = idx {
                    let int_val = f.i();
                    use rug::ops::DivRounding;
                    let modulus = f.modulus();
                    let half_modulus = modulus.clone().div_floor(Integer::from(2));

                    use std::cmp::Ordering;
                    if let Ordering::Greater = int_val.cmp(&half_modulus) {
                        let negative_val = int_val.clone() - modulus;
                        if let Some(neg_i64) = negative_val.to_i64() {
                            panic!("IR Array index {} represents negative number {}. \
                                   Array indices must be non-negative. \
                                   This often happens from expressions like 'arr[i-1]' when i=0 in Circom functions.",
                                  idx, neg_i64);
                        }
                    }
                }
                panic!("IR Array index {} not convertible to usize", idx)
            },
        }
    }
...
impl Value {
    ...
    pub fn as_usize(&self) -> Option<usize> {
        match &self {
            ...
            Value::Field(f) => {
                let int_val = f.i();
                // Check if this represents a negative number
                // In field arithmetic, values in the upper half of the field range
                // typically represent negative numbers
                use rug::ops::DivRounding;
                let modulus = f.modulus();
                let half_modulus = modulus.clone().div_floor(Integer::from(2));

                // Use cmp() for comparison since PartialOrd isn't implemented for Integer/Integer
                use std::cmp::Ordering;
                match int_val.cmp(&half_modulus) {
                    Ordering::Greater => {
                        // This field element represents a negative number
                        let negative_val = int_val.clone() - modulus;
                        if let Some(neg_i64) = negative_val.to_i64() {
                            warn!(
                                "Field element {} represents negative number {} as array index",
                                int_val, neg_i64
                            );
                        }
                        None  // Cannot use as array index
                    },
                    _ => int_val.to_usize()
                }
            },
...
impl ComputationMetadata {
    ...
    /// Iterate over all variables' metadata.
    pub fn vars_iter(&self) -> impl Iterator<Item = &VariableMetadata> {
        self.vars.values()
    }
