...
struct ZGen<'ast> {
    ...
    fn_call_memoization: RefCell<HashMap<FnCallImplInput, (T, Vec<Term>)>>,
}
...
impl<'ast> ZGen<'ast> {
    ...
    fn assembly_assign_with_val_<const IS_CNST: bool>(
        &self,
    ...
    fn function_call_impl_<const IS_CNST: bool>(
        ...
        let cached_entry = self.fn_call_memoization.borrow().get(&input).cloned();

        let ret = if let Some((value, cached_assertions)) = cached_entry {
            // Replay cached assertions so they're not dropped
            self.assertions.borrow_mut().extend(cached_assertions);
            Ok(value)
        } else {
            debug!("successfully memoized {} {:?}", f_name, f_path);
            let asserts_before = self.assertions.borrow().len();
            let result = self.function_call_impl_inner_::<IS_CNST>(
                f,
                args,
                generics,
                f_path.clone(),
            ...
            if let Ok(v) = &result {
                // Capture assertions added during this function call
                let new_assertions = self.assertions.borrow()[asserts_before..].to_vec();
                self.fn_call_memoization
                    .borrow_mut()
                    .insert(input, (v.clone(), new_assertions));
            }
            result
        };
        ...
        match e {
            ast::Expression::Ternary(u) => {
                match self
                    ...
                    .map(const_bool_simple)
                {
                    Ok(Some(true)) => self.expr_impl_::<IS_CNST>(&u.consequence),
                    Ok(Some(false)) => self.expr_impl_::<IS_CNST>(&u.alternative),
                    Err(e) if IS_CNST => Err(e),
                    Ok(None) if IS_CNST => Err("ternary condition not const bool".to_string()),
                    _ => {
            ...
            ast::Expression::IfElse(u) => {
                ...
                {
                    Ok(Some(true)) => {
                        for s in &u.consequence_statements {
                            self.stmt_impl_::<IS_CNST>(s)?;
                        }
                        self.expr_impl_::<IS_CNST>(&u.consequence)
                    }
                    Ok(Some(false)) => {
                        for s in &u.alternative_statements {
                            self.stmt_impl_::<IS_CNST>(s)?;
                        }
                        self.expr_impl_::<IS_CNST>(&u.alternative)
                    }
                    Err(e) if IS_CNST => Err(e),
                    Ok(None) if IS_CNST => Err("IfElse condition not const bool".to_string()),
                    _ => {
                        let c = self.expr_impl_::<false>(&u.condition)?;
                        let cbool = bool(c.clone())?;
                        self.circ_enter_condition(cbool.clone());
                        for s in &u.consequence_statements {
                            self.stmt_impl_::<false>(s)?;
                        }
                        let a = self.expr_impl_::<false>(&u.consequence)?;
                        self.circ_exit_condition();
                        self.circ_enter_condition(term![NOT; cbool]);
                ...
                for inner in &a.inner {
                    match inner {
                        ast::AssemblyStatementInner::Assignment(l) => {
                            // Evaluate RHS once to avoid double evaluation (ZKC-03)
                            let rhs_val = self.expr_impl_::<IS_CNST>(&l.expression)?;
                            self.assembly_assign_with_val_::<IS_CNST>(l, rhs_val.clone())?;
                            if matches!(l.operator, ast::AssignmentOperator::AssignConstrain(_)) {
                                // Evaluate LHS and constrain to equal the already-evaluated RHS
                                let lhs_expr = self.assignee_to_expression(&l.assignee);
                                let lhs_val = self.expr_impl_::<IS_CNST>(&lhs_expr)?;
                                let eq_expr = term![EQ; lhs_val.term, rhs_val.term];
                                self.assert(eq_expr)?;
                            }
                        }
