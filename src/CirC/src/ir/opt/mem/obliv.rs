                    ...
                    if let Some(aa) = self.tups.get(a) {
                        if suitable_const(i) {
                            if matches!(check(aa), Sort::Tuple(..)) {
                                ...
                                // Validate index is within tuple bounds
                                if let Sort::Tuple(ref field_sorts) = check(aa) {
                                    if ii >= field_sorts.len() {
                                        panic!(
                                            "Obliviousness optimization error: Attempted to update field {} of {}-element tuple.\n\
                                             Valid field indices: 0..{}\n\
                                             Tuple sort: {:?}\n\
                                             This indicates an off-by-one error in array-to-tuple conversion or index calculation.",
                                            ii, field_sorts.len(), field_sorts.len(), check(aa)
                                        );
                                    }
                                } else {
                                    panic!(
                                        "Obliviousness optimization error: Expected tuple but got {:?}\n\
                                         Cannot apply update to non-tuple type.",
                                        check(aa)
                                    );
                                }

                                Some(
                                    term![Op::Update(ii); aa.clone(), self.get_t(v).clone()],
                                )
                            } else {
                                None
                            }
                ...
                if let Some(aa) = self.tups.get(a) {
                    if suitable_const(i) {
                        if matches!(check(aa), Sort::Tuple(..)) {
                            ...
                            let index = get_const(i);

                            // Validate index is within tuple bounds
                            if let Sort::Tuple(ref field_sorts) = check(aa) {
                                if index >= field_sorts.len() {
                                    panic!(
                                        "Obliviousness optimization error: Attempted to access field {} of {}-element tuple.\n\
                                         Valid field indices: 0..{}\n\
                                         Tuple sort: {:?}\n\
                                         This indicates an off-by-one error in array-to-tuple conversion or index calculation.",
                                        index, field_sorts.len(), field_sorts.len(), check(aa)
                                    );
                                }
                            } else {
                                panic!(
                                    "Obliviousness optimization error: Expected tuple but got {:?}\n\
                                     Cannot apply field access to non-tuple type.",
                                    check(aa)
                                );
                            }

                            let tt = term![Op::Field(index); aa.clone()];
                            (
                                Some(tt.clone()),
