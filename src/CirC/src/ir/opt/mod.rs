...
pub fn opt<I: IntoIterator<Item = Opt>>(mut cs: Computations, optimizations: I) -> Computations {
    let timings = std::env::var("CIRC_OPT_TIMINGS").is_ok();
    ...
    for i in optimizations {
        let t_pass = if timings { Some(Instant::now()) } else { None };
        debug!("Applying: {:?}", i);

        if let Opt::Link = i {
            link::link_all_function_calls(&mut cs);
            if let Some(t) = t_pass {
                eprintln!("timing: opt {:?} {}ms", i, t.elapsed().as_millis());
            }
            continue;
        }
        ...
        for (_, c) in cs.comps.iter_mut() {
            match i.clone() {
                ...
                Opt::ScalarizeVars => {
                    if scalarize_vars::needs_scalarize_inputs(c) {
