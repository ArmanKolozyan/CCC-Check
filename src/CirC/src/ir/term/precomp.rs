...
impl PreComp {
    ...
    /// If the output already exists, it will be updated.
    pub fn add_output(&mut self, name: String, value: Term) {
        let sort = check(&value);
        let old = self.outputs.insert(name.clone(), value);
        if old.is_none() {
            // New output - add to sequence
            self.sequence.push((name, sort));
        } else {
            // Existing output - update in sequence if sort changed
            if let Some(entry) = self.sequence.iter_mut().find(|(n, _)| n == &name) {
                entry.1 = sort;
            }
        }
    }
