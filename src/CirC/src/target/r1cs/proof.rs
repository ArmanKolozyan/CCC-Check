...
fn value_map_from_path<P: AsRef<Path>>(path: P) -> std::io::Result<HashMap<String, Value>> {
    let mut map = parse_value_map(&std::fs::read(path)?);
    // Accept unqualified inputs from .pin/.in files by aliasing them to `main.*`.
    // Circom examples ship unqualified names (e.g. `a`, `b`) while the IR uses
    // fully qualified names (e.g. `main.a`, `main.b`).
    let mut prefixed = Vec::new();
    for (name, val) in &map {
        if !name.contains('.') {
            let qualified = format!("main.{name}");
            if !map.contains_key(&qualified) {
                prefixed.push((qualified, val.clone()));
            }
        }
    }
    for (name, val) in prefixed {
        map.insert(name, val);
    }
    Ok(map)
}
...
mod test {
    ...
    mod mirage {
        ...
        fn bool_np() {
            ...
            test_setup_prove_verify::<Mirage<halo2curves::bls12381::Bls12381>>(
                c, p_input, v_input,
            );
        }
