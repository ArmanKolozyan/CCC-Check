...
impl LinReducer {
    ...
    fn sub_in(&mut self, var: Var, val: &Lc, con_id: usize) -> bool {
        ...
        let mut removed_vars: Vec<Var> = Vec::new();
        ...
        if !removed_vars.is_empty() {
            // Remove stale uses when a monomial cancels in all three LCs.
            for v in removed_vars {
                if !a.monomials.contains_key(&v)
                    && !b.monomials.contains_key(&v)
                    && !c.monomials.contains_key(&v)
                {
                    self.uses.get_mut(&v).unwrap().remove(&con_id);
                }
            }
        }
        if change {
    ...
    fn run(mut self) -> R1cs {
        while let Some(con_id) = self.queue.pop() {
            if let Some((var, lc)) = as_linear_sub(&self.r1cs.constraints[con_id]) {
                if lc.monomials.len() < self.lc_size_thresh {
                    ...
                    // Move out the use set to avoid cloning large HashSets.
                    let use_ids = std::mem::take(self.uses.get_mut(&var).unwrap());
...
fn find_elim_var(c: &Lc) -> Option<Var> {
    let mut first_final: Option<Var> = None;
    let mut has_chall = false;
    let mut max_round_num: Option<usize> = None;
    let mut max_round_var: Option<Var> = None;

    for v in c.monomials.keys() {
        match v.ty() {
            VarType::FinalWit => {
            ...
            VarType::RoundWit => {
                let n = v.number();
                if max_round_num.map_or(true, |m| n > m) {
                    max_round_num = Some(n);
                    max_round_var = Some(*v);
                }
            }
...
fn as_linear_sub((a, b, c): &(Lc, Lc, Lc)) -> Option<(Var, Lc)> {
    if a.is_zero() || b.is_zero() {
        let elim_var = find_elim_var(c)?;
        let mut lc = c.clone();
        let v = lc.monomials.remove(&elim_var).unwrap();
        lc *= v.recip();
        Some((elim_var, -lc))
    } else {
        None
