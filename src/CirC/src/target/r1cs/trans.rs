...
impl<'cfg> ToR1cs<'cfg> {
    ...
    fn embed_any(&mut self, t: &Term) -> EmbeddedTerm {
        self.embed(t.clone());
        self.cache
            .get(t)
    ...
    fn comp_term<F>(&self, build: F) -> Term
    where
        F: FnOnce() -> Term,
    {
        if self.r1cs.count_only {
            self.zero.0.clone()
        } else {
            build()
        }
    ...
    fn embed_value(&mut self, v: &Value) -> EmbeddedTerm {
        match v {
            Value::Tuple(vs) => EmbeddedTerm::Tuple(vs.iter().map(|v| self.embed_value(v)).collect()),
            Value::Array(..) => {
                panic!("Cannot embed array constant without tuple elimination")
            }
            _ => {
                let t = const_(v.clone());
                self.embed(t.clone());
                self.cache
                    .get(&t)
                    .unwrap_or_else(|| panic!("Missing wire for {:?}", t))
    ...
    fn bv_uint_from_embedded(&mut self, bv: &Rc<RefCell<BvEntry>>) -> (TermLc, usize) {
        let width = bv.borrow().width;
        if let Some(uint) = bv.borrow().uint.clone() {
            return (uint, width);
        }
        let bits = bv.borrow().bits.clone();
        if bits.is_empty() {
            panic!("Bitvector entry missing bits and uint");
        }
        let uint = self.debitify(bits.into_iter(), false);
        bv.borrow_mut().uint = Some(uint.clone());
        (uint, width)
    }

    fn ite_embedded(&mut self, c: &TermLc, t: EmbeddedTerm, f: EmbeddedTerm) -> EmbeddedTerm {
        match (t, f) {
            (EmbeddedTerm::Bool(t), EmbeddedTerm::Bool(f)) => {
                EmbeddedTerm::Bool(self.ite(c.clone(), t, &f))
            }
            (EmbeddedTerm::Field(t), EmbeddedTerm::Field(f)) => {
                EmbeddedTerm::Field(self.ite(c.clone(), t, &f))
            }
            (EmbeddedTerm::Bv(t), EmbeddedTerm::Bv(f)) => {
                let (t_uint, width) = self.bv_uint_from_embedded(&t);
                let (f_uint, f_width) = self.bv_uint_from_embedded(&f);
                assert_eq!(width, f_width);
                let ite = self.ite(c.clone(), t_uint, &f_uint);
            ...
            (EmbeddedTerm::Tuple(ts), EmbeddedTerm::Tuple(fs)) => {
                assert_eq!(ts.len(), fs.len());
                EmbeddedTerm::Tuple(
                    ts.into_iter()
                        .zip(fs.into_iter())
                        .map(|(t, f)| self.ite_embedded(c, t, f))
                        .collect(),
                )
            }
            _ => panic!("Tuple ITE type mismatch"),
        }
    }

    fn embed_tuple_var_elem(
        &mut self,
        name: &str,
        term: &Term,
        sort: &Sort,
        ...
        match sort {
            Sort::Bool => {
                let comp = self.comp_term(|| {
                    term![Op::Ite; term.clone(), self.one.0.clone(), self.zero.0.clone()]
                });
                let lc = self.fresh_var(name, comp, ty);
                if !public {
                    self.enforce_bit(lc.clone());
                }
                EmbeddedTerm::Bool(lc)
            }
            Sort::Field(f) => {
                assert_eq!(f, &self.field);
                let lc = self.fresh_var(name, term.clone(), ty);
                EmbeddedTerm::Field(lc)
            }
            Sort::BitVector(n) => {
                let comp = self
                    .comp_term(|| term![Op::new_ubv_to_pf(self.field.clone()); term.clone()]);
                let lc = self.fresh_var(name, comp, ty);
                let mut entry = BvEntry {
                    width: *n,
                    uint: Some(lc.clone()),
                    bits: Vec::new(),
                };
                if !public {
                    entry.bits = self.bitify("tuple_bv", &lc, *n, false);
                }
                EmbeddedTerm::Bv(Rc::new(RefCell::new(entry)))
            }
            Sort::Tuple(sorts) => EmbeddedTerm::Tuple(
                sorts
                    .iter()
                    .enumerate()
                    .map(|(i, sort)| {
                        let field_term = term![Op::Field(i); term.clone()];
                        let field_name = format!("{name}.{i}");
                        self.embed_tuple_var_elem(&field_name, &field_term, sort, ty, public)
                    })
                    .collect(),
            ),
            Sort::Array(..) => {
                panic!("Cannot embed array tuple element without linear scan")
            }
            _ => panic!("Unsupported sort in tuple var element: {:?}", sort),
        }
    }

    fn embed_tuple_var(&mut self, var: &Term, ty: VarType) {
        if let Sort::Tuple(sorts) = check(var) {
            let public = matches!(ty, VarType::Inst);
            let base_name = var.as_var_name().to_string();
            let elems = sorts
                .iter()
                .enumerate()
                .map(|(i, sort)| {
                    let field_term = term![Op::Field(i); var.clone()];
                    let field_name = format!("{base_name}.{i}");
                    self.embed_tuple_var_elem(&field_name, &field_term, sort, ty, public)
                })
                .collect();
            self.cache.insert(var.clone(), EmbeddedTerm::Tuple(elems));
            self.embed.borrow_mut().insert(var.clone());
        } else {
            panic!("embed_tuple_var called on non-tuple");
        }
    }
    ...
    fn embed_tuple(&mut self, t: Term) {
        ...
        let embedded = match t.op() {
            Op::Const(v) => self.embed_value(v),
            Op::Tuple => EmbeddedTerm::Tuple(t.cs().iter().map(|c| self.embed_any(c)).collect()),
            Op::Update(i) => {
                let mut base = match self.embed_any(&t.cs()[0]) {
                    EmbeddedTerm::Tuple(v) => v,
                    _ => panic!("Update on non-tuple {}", t.cs()[0]),
                };
                if *i >= base.len() {
                    panic!("Tuple update index {} out of bounds {}", i, base.len());
                }
                base[*i] = self.embed_any(&t.cs()[1]);
                EmbeddedTerm::Tuple(base)
            }
            Op::Ite => {
                self.embed(t.cs()[0].clone());
                let cond = self.get_bool(&t.cs()[0]).clone();
                let t_val = self.embed_any(&t.cs()[1]);
                let f_val = self.embed_any(&t.cs()[2]);
                self.ite_embedded(&cond, t_val, f_val)
            }
            _ => panic!("Cannot embed tuple term: {}", t),
        };
        self.cache.insert(t.clone(), embedded);
        self.embed.borrow_mut().insert(t);
    }
    ...
    fn decomp<D: Display + ?Sized>(&mut self, d: &D, x: &TermLc, n: usize) -> Vec<TermLc> {
        (0..n)
            .map(|i| {
                ...
                self.fresh_bit(&format!("{d}_b{i}"), comp)
            })
            .collect::<Vec<_>>()
    }
    ...
    fn mul(&mut self, mut a: TermLc, mut b: TermLc) -> TermLc {
        ...
        let a_term = a.0.clone();
        let b_term = b.0.clone();
        let mul_val = self.comp_term(|| term![PF_MUL; a_term, b_term]);
    ...
    fn embed_var(&mut self, var: &Term, ty: VarType) {
        ...
        match var.op() {
            Op::Var(v) if matches!(&v.sort, Sort::Bool) => {
                let comp = self.comp_term(|| {
                    term![Op::Ite; var.clone(), self.one.0.clone(), self.zero.0.clone()]
                });
            ...
            Op::Var(v) if v.sort.is_bv() => {
                let public = matches!(ty, VarType::Inst);
                let comp =
                    self.comp_term(|| term![Op::new_ubv_to_pf(self.field.clone()); var.clone()]);
                let lc = self.fresh_var(&v.name, comp, ty);
            ...
            Op::Var(v) if matches!(&v.sort, Sort::Tuple(_)) => {
                if !self.r1cs.count_only {
                    panic!("Cannot embed tuple var without tuple elimination");
                }
                self.embed_tuple_var(var, ty);
            }
                ...
                Op::PfNaryOp(o) => {
                    ...
                    match o {
                        PfNaryOp::Add => {
                            ...
                            if lc.1.monomial_count() > self.cfg.r1cs.lc_elim_thresh {
