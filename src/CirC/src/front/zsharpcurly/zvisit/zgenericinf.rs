...
impl<'ast, 'gen, const IS_CNST: bool> ZGenericInf<'ast, 'gen, IS_CNST> {
    ...
    pub fn unify_generic<ATIter: Iterator<Item = Ty>>(
        ...
        let g_names = self
            ...
            .collect::<Vec<_>>();
        let mut solved = if let Some(t) = self.constr.as_ref() {
            find_unique_model(t, g_names.clone()).unwrap_or_else(|| {
                panic!(
                    "Generic inference failed: no unique solution \
                     for generics [{}] with suffix '{}'",
                    self.gens.iter().map(|g| g.value.as_str()).collect::<Vec<_>>().join(", "),
                    self.sfx
                )
            })
        } else {
