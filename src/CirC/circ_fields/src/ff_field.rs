        ...
        pub mod $name {
            ...
            impl std::convert::From<Integer> for Ft {
                ...
                fn from(mut i: Integer) -> Self {
                    ...
                    // Reduce modulo field if needed
                    if i.significant_digits::<u8>() > len || i.cmp0() == Less {
                        i.rem_floor_assign(&*FMOD);
                    }

                    // Extra reduction to ensure value is in [0, FMOD)
                    // This handles edge cases where rem_floor_assign leaves the value
                    // at exactly FMOD or slightly above due to rounding
                    let max_iterations = 10;  // Safety limit
                    let mut iterations = 0;
                    while i >= *FMOD && iterations < max_iterations {
                        i -= &*FMOD;
                        iterations += 1;
                    }
                    while i < 0 && iterations < max_iterations {
                        i += &*FMOD;
                        iterations += 1;
                    }
                    ...
                    if iterations >= max_iterations {
                        panic!(
                            "Failed to reduce integer to field range after {} iterations\n\
                             \n\
                             Integer value: {}\n\
                             Field modulus: {}\n\
                             This indicates a bug in field reduction logic.",
                            max_iterations, i, *FMOD
                        );
                    }

                    debug_assert!(
                        i >= 0 && i < *FMOD,
                        "rem_floor_assign produced out-of-range result: {} (modulus: {})",
                        i, *FMOD
                    );

                    // Write to bytes (LSB first)
                    let mut bytes = [0u8; 8 * $limbs];
                    i.write_digits(&mut bytes, Order::Lsf);

                    // Try to construct field element
                    Self::from_repr_vartime(FtRepr(bytes)).unwrap_or_else(|| {
                        ...
                        panic!(
                            "Failed to create field element from integer after proper reduction\n\
                             \n\
                             Integer value: {}\n\
                             Integer in hex: {:#x}\n\
                             Bytes (LSB-first, hex): {}\n\
                             ...
                             Field size: {} bits\n\
                             \n\
                             The integer was properly reduced to [0, modulus), but from_repr_vartime\n\
                             still returned None. This indicates either:\n\
                             1. A mismatch between the byte representation and field format\n\
                             2. The circuit uses values incompatible with this field\n\
                             3. A bug in the field element construction\n\
                             \n\
                             For Ed25519 or large-integer circuits, consider using:\n\
                             - BN254 field: --field bn254\n\
                             - Custom field matching your circuit's requirements",
                            i, i, hex_bytes, *FMOD, 8 * $limbs * 8
                        )
                    })
                }
            }
