...
module Value_Inferencer.BugDetection.TaggedCircomlib.Num2BitsTest (spec) where
...
spec = describe "Num2Bits Template Test" $ do
  it "successfully completes analysis without detecting bugs" $ do
    ...
    let inp = Binding { name = "in", vid = 0, sort = FieldMod p, tag = Just $ MaxValTag 7 } -- 2^3 - 1 = 7

    -- output bits (binary signals)
    let out0 = Binding { name = "out0", vid = 1, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let out1 = Binding { name = "out1", vid = 2, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let out2 = Binding { name = "out2", vid = 3, sort = FieldMod p, tag = Just $ SimpleTag "binary" }

    -- constraints

    -- binary constraints for each output bit: out[i] * (out[i] - 1) === 0
    let c_out0_binary = EqC 100 (Mul (Var "out0") (Sub (Var "out0") (Int 1))) (Int 0)
    let c_out1_binary = EqC 101 (Mul (Var "out1") (Sub (Var "out1") (Int 1))) (Int 0)
    let c_out2_binary = EqC 102 (Mul (Var "out2") (Sub (Var "out2") (Int 1))) (Int 0)

    -- reconstruction constraint: out0 + 2*out1 + 4*out2 === in
    let c_reconstruction = EqC 103 
          (Add (Add (Var "out0") (Mul (Int 2) (Var "out1"))) (Mul (Int 4) (Var "out2")))
          (Var "in")

    -- all constraints
    let allConstraints = [ c_out0_binary, c_out1_binary, c_out2_binary, c_reconstruction ]
