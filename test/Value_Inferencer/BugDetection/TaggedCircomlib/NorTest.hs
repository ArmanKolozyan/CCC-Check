...
module Value_Inferencer.BugDetection.TaggedCircomlib.NorTest (spec, norTestProgram) where
...
-- | NOR Gate template test program
norTestProgram :: Program
norTestProgram = 
  let
    ...
    c_nor_def = EqC 100 (Var "out") (Sub (Add (Mul (Var "a") (Var "b")) (Int 1)) (Add (Var "a") (Var "b")))

    -- all constraints
    allConstraints = [ c_nor_def ]
...
spec = describe "NOR Gate Template Test" $ do
  it "successfully completes analysis without detecting bugs" $ do
    ...
    let bugResult = detectBugs norTestProgram Nothing
