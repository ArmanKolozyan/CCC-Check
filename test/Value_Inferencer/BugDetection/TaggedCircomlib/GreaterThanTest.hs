...
module Value_Inferencer.BugDetection.TaggedCircomlib.GreaterThanTest (spec) where
...
spec = describe "GreaterThan Template Test" $ do
  it "successfully completes analysis without detecting bugs" $ do
    ...
    let c_lt_diff = EqC 100 (Var "lt_diff") (Sub (Add (Var "in1") (Int 4)) (Var "in0"))
    ...
    let c_output = EqC 105 (Var "out") (Sub (Int 1) (Var "lt_diff_b2"))

    -- all constraints
    let allConstraints = [ c_lt_diff, c_lt_diff_b0_binary, c_lt_diff_b1_binary, c_lt_diff_b2_binary, c_lt_reconstruction, c_output ]
