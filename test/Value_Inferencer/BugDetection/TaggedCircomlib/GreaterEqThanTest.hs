...
module Value_Inferencer.BugDetection.TaggedCircomlib.GreaterEqThanTest (spec) where
...
spec = describe "GreaterEqThan Template Test" $ do
  it "successfully completes analysis without detecting bugs" $ do
    ...
    let lt_diff = Binding { name = "lt_diff", vid = 2, sort = FieldMod p, tag = Just $ MaxValTag 7 }

    -- Num2Bits(n+1) = Num2Bits(3) component outputs for LessThan (binary bits of lt_diff)
    let lt_diff_b0 = Binding { name = "lt_diff_b0", vid = 3, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let lt_diff_b1 = Binding { name = "lt_diff_b1", vid = 4, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let lt_diff_b2 = Binding { name = "lt_diff_b2", vid = 5, sort = FieldMod p, tag = Just $ SimpleTag "binary" }

    -- LessThan output (binary)
    let lt_out = Binding { name = "lt_out", vid = 6, sort = FieldMod p, tag = Just $ SimpleTag "binary" }

    -- final output (binary) - NOT of LessThan output
    let out = Binding { name = "out", vid = 7, sort = FieldMod p, tag = Just $ SimpleTag "binary" }

    -- constraints

    -- LessThan component logic: lt_diff <== in[0] + (1<<n) - in[1] = in[0] + 4 - in[1]
    let c_lt_diff = EqC 100 (Var "lt_diff") (Sub (Add (Var "in0") (Int 4)) (Var "in1"))

    -- Num2Bits constraints for lt_diff (binary constraints)
    let c_lt_diff_b0_binary = EqC 101 (Mul (Var "lt_diff_b0") (Sub (Var "lt_diff_b0") (Int 1))) (Int 0)
    let c_lt_diff_b1_binary = EqC 102 (Mul (Var "lt_diff_b1") (Sub (Var "lt_diff_b1") (Int 1))) (Int 0)
    let c_lt_diff_b2_binary = EqC 103 (Mul (Var "lt_diff_b2") (Sub (Var "lt_diff_b2") (Int 1))) (Int 0)

    -- Num2Bits reconstruction: lt_diff_b0 + 2*lt_diff_b1 + 4*lt_diff_b2 === lt_diff
    let c_lt_reconstruction = EqC 104 
    ...
    let c_lt_output = EqC 105 (Var "lt_out") (Sub (Int 1) (Var "lt_diff_b2"))

    -- NOT component logic: out <== 1 + lt_out - 2*lt_out
    let c_not_output = EqC 106 (Var "out") (Sub (Add (Int 1) (Var "lt_out")) (Mul (Int 2) (Var "lt_out")))

    -- all constraints
    let allConstraints = [ c_lt_diff, c_lt_diff_b0_binary, c_lt_diff_b1_binary, c_lt_diff_b2_binary, c_lt_reconstruction, c_lt_output, c_not_output ]
