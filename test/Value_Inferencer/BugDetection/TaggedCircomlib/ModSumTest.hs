...
module Value_Inferencer.BugDetection.TaggedCircomlib.ModSumTest (spec) where
...
spec = describe "ModSum Template Test" $ do
  it "detects analysis limitation with modular arithmetic bounds" $ do
    ...
    -- template parameter
    let n = 3 -- using 3-bit modular addition for this test

    -- bindings

    -- inputs (two field values with maxbit constraint)
    let a = Binding { name = "a", vid = 0, sort = FieldMod p, tag = Just $ MaxValTag 7 } -- 2^3 - 1 = 7
    let b = Binding { name = "b", vid = 1, sort = FieldMod p, tag = Just $ MaxValTag 7 } -- 2^3 - 1 = 7

    -- intermediate calculation: a + b (can be up to 14)
    let a_plus_b = Binding { name = "a_plus_b", vid = 2, sort = FieldMod p, tag = Just $ MaxValTag 14 }

    -- Num2Bits(n+1) = Num2Bits(4) component outputs (binary bits of a + b)
    let sum_b0 = Binding { name = "sum_b0", vid = 3, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let sum_b1 = Binding { name = "sum_b1", vid = 4, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let sum_b2 = Binding { name = "sum_b2", vid = 5, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let sum_b3 = Binding { name = "sum_b3", vid = 6, sort = FieldMod p, tag = Just $ SimpleTag "binary" }

    -- outputs
    let carry = Binding { name = "carry", vid = 7, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let sum_out = Binding { name = "sum", vid = 8, sort = FieldMod p, tag = Just $ MaxValTag 7 } -- maxbit = n

    -- constraints

    -- a + b
    let c_a_plus_b = EqC 100 (Var "a_plus_b") (Add (Var "a") (Var "b"))

    -- Num2Bits constraints for a + b (binary constraints)
    let c_sum_b0_binary = EqC 101 (Mul (Var "sum_b0") (Sub (Var "sum_b0") (Int 1))) (Int 0)
    let c_sum_b1_binary = EqC 102 (Mul (Var "sum_b1") (Sub (Var "sum_b1") (Int 1))) (Int 0)
    let c_sum_b2_binary = EqC 103 (Mul (Var "sum_b2") (Sub (Var "sum_b2") (Int 1))) (Int 0)
    let c_sum_b3_binary = EqC 104 (Mul (Var "sum_b3") (Sub (Var "sum_b3") (Int 1))) (Int 0)

    -- Num2Bits reconstruction: sum_b0 + 2*sum_b1 + 4*sum_b2 + 8*sum_b3 === a + b
    let c_reconstruction = EqC 105 
    ...
    let c_carry = EqC 106 (Var "carry") (Var "sum_b3")

    -- sum output: sum <== a + b - carry * (1 << n) = a + b - carry * 8
    let c_sum = EqC 107 (Var "sum") (Sub (Var "a_plus_b") (Mul (Var "carry") (Int 8)))

    -- all constraints
    let allConstraints = [ c_a_plus_b, c_sum_b0_binary, c_sum_b1_binary, c_sum_b2_binary, c_sum_b3_binary, c_reconstruction, c_carry, c_sum ]
