...
module Value_Inferencer.BugDetection.TaggedCircomlib.LessEqThanTest (spec) where
...
spec = describe "LessEqThan Template Test" $ do
  it "successfully completes analysis without detecting bugs" $ do
    ...
    let gt_diff = Binding { name = "gt_diff", vid = 2, sort = FieldMod p, tag = Just $ MaxValTag 7 }

    -- Num2Bits(n+1) = Num2Bits(3) component outputs for GreaterThan (binary bits of gt_diff)
    let gt_diff_b0 = Binding { name = "gt_diff_b0", vid = 3, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let gt_diff_b1 = Binding { name = "gt_diff_b1", vid = 4, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    let gt_diff_b2 = Binding { name = "gt_diff_b2", vid = 5, sort = FieldMod p, tag = Just $ SimpleTag "binary" }

    -- GreaterThan output (binary)
    let gt_out = Binding { name = "gt_out", vid = 6, sort = FieldMod p, tag = Just $ SimpleTag "binary" }
    ...
    let c_gt_diff = EqC 100 (Var "gt_diff") (Sub (Add (Var "in1") (Int 4)) (Var "in0"))

    -- Num2Bits constraints for gt_diff (binary constraints)
    let c_gt_diff_b0_binary = EqC 101 (Mul (Var "gt_diff_b0") (Sub (Var "gt_diff_b0") (Int 1))) (Int 0)
    let c_gt_diff_b1_binary = EqC 102 (Mul (Var "gt_diff_b1") (Sub (Var "gt_diff_b1") (Int 1))) (Int 0)
    let c_gt_diff_b2_binary = EqC 103 (Mul (Var "gt_diff_b2") (Sub (Var "gt_diff_b2") (Int 1))) (Int 0)

    -- Num2Bits reconstruction: gt_diff_b0 + 2*gt_diff_b1 + 4*gt_diff_b2 === gt_diff
    let c_gt_reconstruction = EqC 104 
    ...
    let c_gt_output = EqC 105 (Var "gt_out") (Sub (Int 1) (Var "gt_diff_b2"))

    -- NOT component logic: out <== 1 + gt_out - 2*gt_out
    let c_not_output = EqC 106 (Var "out") (Sub (Add (Int 1) (Var "gt_out")) (Mul (Int 2) (Var "gt_out")))

    -- all constraints
    let allConstraints = [ c_gt_diff, c_gt_diff_b0_binary, c_gt_diff_b1_binary, c_gt_diff_b2_binary, c_gt_reconstruction, c_gt_output, c_not_output ]
